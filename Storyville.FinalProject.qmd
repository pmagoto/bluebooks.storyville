...
<<<<<<< HEAD
people.1900 <- read.csv("/Users/paigemagoto/Desktop/bluebooks.storyville/CSVs/people.1900.geocoded.csv")
people.1915 <- read.csv("/Users/paigemagoto/Desktop/bluebooks.storyville/CSVs/people.1915.geocoded.csv")
addresses.1900 <- read.csv("/Users/paigemagoto/Desktop/bluebooks.storyville/CSVs/addresses.1900.geocoded.csv")
addresses.1915 <- read.csv("/Users/paigemagoto/Desktop/bluebooks.storyville/CSVs/addresses.1915.geocoded.csv")
=======
people.1900 <- read.csv("~/Desktop/Magoto-FP/CSVs/addresses.1900.geocoded.csv")
people.1915 <- read.csv("~/Desktop/Magoto-FP/CSVs/people.1915.geocoded.csv")
addresses.1900 <- read.csv("~/Desktop/Magoto-FP/CSVs/addresses.1900.geocoded.csv")
addresses.1915 <- read.csv("~/Desktop/Magoto-FP/CSVs/addresses.1915.geocoded.csv")
race.id.colors <- c("colored" = "gray25", "white" = "white", "octoroon" = "gold")
>>>>>>> 63996302f6039f09ae3e0b5a6590615f6ae58777
```
...
ggplot(case.study.counts, aes(x = year, y = count)) + geom_line(color = "red") + geom_point(color = "red") + geom_text(aes(label = race.id), color = "red", vjust = -.5, size = 2.5)
```

As Willie Piazza's Castle is
...
combine people and addresses for leaflet:

```{r}
test.people.1900 <- read.csv("CSVs/people.1900.geocoded.csv") 
test.people.1915 <- read.csv("CSVs/people.1915.geocoded.csv")


test.addresses.1900 <- read.csv("CSVs/addresses.1900.geocoded.csv") %>% rename("people.id"="id.sexworkers")
##Something is up with this dataframe - each address.id is duplicated several times
test.addresses.1915 <- read.csv("CSVs/addresses.1915.geocoded.csv") %>% rename("people.id"="id.sexworkers") %>% separate_rows(people.id, convert = TRUE)

full.1900.leaflet <- left_join(test.addresses.1900, test.people.1900, by = "address.id") %>% group_by(full.address.y) %>%  # Group by the full address
  mutate(all.names = paste(first.name, last.name, sep=" ", collapse=", ")) %>% distinct(street.address.y, .keep_all = TRUE)

test.addresses.1915 <- test.addresses.1915 %>% distinct(address.id, .keep_all = TRUE)
buisiness.1915<- left_join(test.addresses.1915, test.people.1915, by = "address.id") %>% group_by(full.address.y) %>%  # Group by the full address
...
##AR - The repitition is coming from the dataframe not the map - this code is good.
...
leaflet(data=buisiness.1915) %>% 
  ...
  addMarkers(~lon, ~lat, popup = paste(
      "<b>This business is named:</b> ", buisiness.1915$buisiness.name, 
