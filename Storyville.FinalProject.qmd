...
full.1900.leaflet <- left_join(test.people.1900, test.addresses.1900, by = "address.id") %>% ungroup() %>% select(street.address.x, race.id.y, buisness.name, alldistinct(street.address.x)
```

\

```{#buisiness.name, street.address.x, lat.x, lon.y)}
